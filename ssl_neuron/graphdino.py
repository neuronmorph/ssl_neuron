...
class GraphTransformer(nn.Module):
    ...
    def forward(self, node_feat, adj, lapl):
       ...
       # print(B,N,_)
        ...
        x = torch.cat((node_feat, eucl, pos_embedding_pstep_token), dim=2)
        x = self.PE(x)
        x = torch.cat((cls_tokens, x), dim=1)
...
class PStepRWEncoding(nn.Module):
    def __init__(self, output_dim=32, hidden_dim=128, p=1, use_edge_attr=False, normalization=None):
        ...
        self.mlp = MLP_general(input_dim=input_dim, output_dim=output_dim, hidden_dim=hidden_dim).to(torch.device("cuda:0"))
